...
// Разрешаем CORS (Render URL фронтенда должен быть в ALLOWED_ORIGINS или SITE_URL)
...
app.use(
  cors({
    origin: (origin, cb) => {
      // Разрешаем запросы без origin (например, от самой Робокассы или Postman)
      if (
        !origin ||
...
// Временное хранилище (в идеале заменить на базу данных типа PostgreSQL/MongoDB)
...
function buildCommands(itemId, itemType, nick) {
  ...
  if (itemType === "rank") {
    ...
    // Выдача Battle Pass (как вы просили)
    rankName = "bp";
    cmds.push(`lp user ${nick} permission set BetterBattlePass.perks.vip`);
  } else if (itemType === "cosmetic") {
    cmds.push(`givepet ${nick} ${itemId}`); // Заменить на вашу команду выдачи косметики
  }
...
app.post("/create-payment", payLimiter, (req, res) => {
  ...
  // Робокасса принимает InvId только в виде 32-bit Integer (макс ~2.1 млрд). Date.now() сломает платеж!
  const invId = Math.floor(Date.now() / 1000);
  const sum = parseFloat(price).toFixed(2);
  const desc = `${buildItemLabel(itemId, itemType)} for ${nick}`;

  // shp_ параметры обязаны быть по алфавиту: shp_item, shp_nick, shp_type
...
app.post("/robokassa/result", resLimiter, async (req, res) => {
  ...
  try {
    p.tgMsgId = await sendPurchaseNotification(p);
  } catch (e) {
    console.warn("TG error:", e.message);
  }

  // Робокасса требует именно такой ответ!
  res.send(`OK${InvId}`);
});
