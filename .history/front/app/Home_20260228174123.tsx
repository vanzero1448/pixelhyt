...
:root{
  ...
  --blue:#3b82f6;--blue-dim:rgba(59,130,246,.12);--line:rgba(255,255,255,.06);
...
.h-scrim{position:absolute;inset:0;z-index:1;background:linear-gradient(to right,rgba(7,7,11,.92) 0%,rgba(7,7,11,.6) 55%,rgba(7,7,11,.1) 100%)}
.h-scrim2{position:absolute;bottom:0;left:0;right:0;height:160px;z-index:1;background:linear-gradient(to bottom,transparent,var(--bg))}
.h-hero-inner{position:relative;z-index:2;max-width:1180px;margin:0 auto;padding:0 28px;width:100%}
.h-pill{display:inline-flex;align-items:center;gap:8px;border:1px solid var(--w2);border-radius:999px;padding:5px 14px 5px 8px;background:rgba(255,255,255,.04);backdrop-filter:blur(10px);font-family:var(--fm);font-size:10px;letter-spacing:.1em;text-transform:uppercase;color:var(--t2);margin-bottom:24px}
.h-pill-dot{width:6px;height:6px;border-radius:50%;background:#22c55e;box-shadow:0 0 8px #22c55e;animation:pdot 2s ease-in-out infinite}
@keyframes pdot{0%,100%{opacity:1}50%{opacity:.4}}
.h-h1{font-family:var(--fh);font-size:clamp(52px,8vw,88px);font-weight:800;line-height:.92;letter-spacing:-.04em;color:#fff;margin-bottom:22px}
.h-h1 em{font-style:normal;color:var(--blue);position:relative}
.h-h1 em::after{content:'';position:absolute;bottom:-4px;left:0;right:0;height:2px;background:var(--blue);opacity:.35;border-radius:2px}
...
.h-scroll{position:absolute;bottom:32px;left:50%;transform:translateX(-50%);z-index:2;width:1px;height:48px;background:linear-gradient(to bottom,rgba(255,255,255,.3),transparent);opacity:.4}
.btn{display:inline-flex;align-items:center;gap:7px;border-radius:9px;font-family:var(--fb);font-weight:600;cursor:pointer;border:1px solid transparent;transition:all .2s;white-space:nowrap}
...
.h-tag{display:inline-flex;align-items:center;gap:5px;font-family:var(--fm);font-size:9px;letter-spacing:.16em;text-transform:uppercase;color:var(--t3);border:1px solid var(--w);border-radius:4px;padding:3px 9px;margin-bottom:12px}
.h-sh h2{font-family:var(--fh);font-size:clamp(26px,4vw,36px);font-weight:800;letter-spacing:-.03em;line-height:1.05}
.h-feats{display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:8px;margin-bottom:56px}
.h-feat{background:var(--s1);border:1px solid var(--w);border-radius:12px;padding:20px;transition:border-color .25s,transform .3s cubic-bezier(.34,1.2,.64,1)}
.h-feat:hover{transform:translateY(-4px)}
.h-feat-icon{width:38px;height:38px;border-radius:9px;display:flex;align-items:center;justify-content:center;margin-bottom:14px;border:1px solid}
...
.h-card{background:var(--s1);border:1px solid var(--w);border-radius:14px;overflow:hidden;margin-bottom:12px;transition:border-color .3s}
...
.h-card-img{width:190px;flex-shrink:0;background:var(--s2);position:relative;display:flex;align-items:center;justify-content:center;padding:28px 16px;overflow:hidden}
.h-card-img-glow{position:absolute;inset:0;background:radial-gradient(ellipse at 50% 80%,rgba(59,130,246,.14) 0%,transparent 70%)}
.h-card-img img{width:100%;max-width:140px;height:auto;object-fit:contain;position:relative;z-index:1;animation:float 5s ease-in-out infinite;filter:drop-shadow(0 20px 40px rgba(59,130,246,.3))}
...
.h-online{display:flex;align-items:center;gap:7px;background:rgba(34,197,94,.07);border:1px solid rgba(34,197,94,.2);border-radius:8px;padding:6px 12px}
.h-online-dot{width:6px;height:6px;border-radius:50%;background:#22c55e;box-shadow:0 0 6px #22c55e;animation:pdot 2s ease-in-out infinite}
...
.h-stat-box{background:var(--s2);border:1px solid var(--w);border-radius:8px;padding:8px 14px;display:flex;flex-direction:column;align-items:center;min-width:60px}
.h-stat-box-val{font-family:var(--fh);font-size:17px;font-weight:800;letter-spacing:-.035em;color:var(--t1);line-height:1}
.h-stat-box-lbl{font-family:var(--fm);font-size:9px;font-weight:600;color:var(--t3);text-transform:uppercase;letter-spacing:.08em;margin-top:3px}
.h-ip-row{display:flex;align-items:center;gap:10px;background:var(--s2);border:1px solid var(--w);border-radius:8px;padding:10px 14px}
.h-ip-label{font-family:var(--fm);font-size:10px;letter-spacing:.1em;text-transform:uppercase;color:var(--t3);flex-shrink:0}
.h-ip-val{flex:1;font-family:var(--fm);font-size:13px;font-weight:600;color:var(--t1);letter-spacing:.04em}
.h-card-actions{display:flex;gap:8px;flex-wrap:wrap}
.h-coming{border:1px dashed rgba(255,255,255,.07);border-radius:12px;padding:14px 20px;display:flex;justify-content:space-between;align-items:center;color:var(--t3)}
.h-cta{background:var(--s1);border-top:1px solid var(--line);border-bottom:1px solid var(--line);padding:72px 0;text-align:center}
.h-cta h2{font-family:var(--fh);font-size:clamp(28px,4vw,42px);font-weight:800;letter-spacing:-.03em;margin-bottom:12px}
...
.h-ov{position:fixed;inset:0;z-index:900;background:rgba(0,0,0,.88);backdrop-filter:blur(20px);display:flex;align-items:center;justify-content:center;padding:20px;animation:hfo .18s ease}
@keyframes hfo{from{opacity:0}to{opacity:1}}
.h-modal{background:var(--s1);border:1px solid var(--w);border-radius:18px;width:100%;max-width:540px;max-height:90vh;overflow-y:auto;animation:hsu .28s cubic-bezier(.34,1.1,.64,1);scrollbar-width:none}
...
.h-modal-close{background:var(--s2);border:1px solid var(--w);border-radius:8px;padding:7px;cursor:pointer;color:var(--t3);line-height:1;transition:all .2s}
...
.h-step{display:flex;gap:14px;align-items:flex-start;background:var(--s2);border:1px solid var(--w);border-radius:10px;padding:14px 16px;transition:border-color .2s}
.h-step:hover{border-color:var(--w2)}
.h-step-num{width:34px;height:34px;border-radius:8px;flex-shrink:0;display:flex;align-items:center;justify-content:center;font-family:var(--fm);font-size:11px;font-weight:700;letter-spacing:.04em;border:1px solid}
.h-step-title{font-weight:700;font-size:14px;margin-bottom:4px}
.h-step-desc{font-size:13px;color:var(--t2);line-height:1.6}
.h-step-code{display:inline-flex;align-items:center;gap:6px;margin-top:8px;background:var(--s1);border:1px solid var(--w);border-radius:6px;padding:4px 10px;font-family:var(--fm);font-size:12px;letter-spacing:.04em}
          ...
          <div style={{ maxWidth: 580 }}>
            <div className="fade-in d1">
              <div className="h-pill">
                <div className="h-pill-dot" /> Server is Online · Hytale ·
                Season 1
              </div>
            </div>
          ...
          <div className="h-feats">
            {[
              ...
              },
              {
                ...
                color: "#f43f5e",
                name: "24/7 Support",
                desc: "Fast, real human support via Telegram. Most issues resolved under 10 minutes.",
              },
            ].map((f) => (
              <div
                key={f.name}
            ...
            <div className="h-card-body">
              ...
              <div className="h-card-content">
                ...
                <div className="h-ip-row">
                  ...
                  <span className="h-ip-val">{SERVER_IP}</span>
                  <button
